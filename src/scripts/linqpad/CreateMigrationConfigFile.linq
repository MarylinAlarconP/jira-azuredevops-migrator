...
void Main()
{
	var (excelFilePath, outputJsonPath) = GetFilePaths("JiraAzureFieldMappings.xlsx", "config-scrum-philips-autogen.json");
...
Root CreateJsonObject(List<TypeMapping> typeMappings, List<CombinedFieldMapping> fieldMappings, List<LinkMapping> linkMappings)
{
	return new Root
    {
        ...
        BaseAreaPath = "Playground/Business Systems/Salesforce",
        BaseIterationPath = "Playground/2026",
        BatchSize = 20,
        DownloadOptions = 0,
        ...
        LogLevel = "Info",
        ProcessTemplate = "SAL",
        Query ="project = SAL AND issuetype in (Bug, Spike, Story, Task, Sub-task, Epic) AND Key in (SAL-3689) ORDER BY labels DESC, parent ASC, key ASC",
        SleepTimeBetweenRevisionImportMilliseconds = 0,
        SourceProject = "SAL",
        SprintField = "Sprint",
        SuppressNotifications = false,
        TargetProject = "Playground",
        TypeMap = new TypeMap { Type = typeMappings },
        UserMappingFile = @"D:\temp1\jira-azure-migration\configuration\user_emails.txt", 
        UsingJiraCloud = true,
        Workspace = @"D:\temp1\jira-azure-migration\workspace\"
    };
}

(string excelFilePath, string outputJsonPath) GetFilePaths(string mappingFilename, string outputFilename)
{
	var tempDirectory = @"D:\temp1\jira-azure-migration\configuration";
